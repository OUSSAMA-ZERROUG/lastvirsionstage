...
import { DataEquipmentTable} from '@/components/DataEquipmentTable';
import { columns, User } from './columns';
import{ equipmentColumns,Equipment } from './equipmentColumns'
...
export default function page() {
  ...
  const [datax, setDatax] = useState<Equipment[]>([]); // Define dataz as an array of users
  ...
  async function getEquipmentFromAPI() {
    try {
      ...
      const responseData = await res.json();
      setDatax(responseData.data); // Update data in state
    } catch (error) {
      console.error('Error fetching data:', error);
      // Optionally, display an error message to the user
    }
  ...
  return (
    <section className="py-24">
      ...
      <DataEquipmentTable columns={equipmentColumns} data={datax} />
      </div>
    </section>
  );
}